// frontend/src/components/DeliveryConfirmationPage.tsx -> CÓDIGO FINAL E CORRIGIDO
...
const DeliveryConfirmationPage = ({ onConfirmClick, API_URL, refreshKey }: DeliveryConfirmationPageProps) => {
  ...
  }, [fetchPendingMovements, refreshKey, searchTerm, searchType]); // A dependência está correta
              ...
              <tbody>
                {pendingMovements.map((movement) => (
                  <tr key={movement.id} className="bg-white border-b hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium text-gray-900">{movement.recipient_display_name}</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <button onClick={() => onConfirmClick(movement)} className="bg-green-600 text-white px-3 py-1 rounded-lg shadow-md hover:bg-green-700 whitespace-nowrap text-xs flex items-center mx-auto">
