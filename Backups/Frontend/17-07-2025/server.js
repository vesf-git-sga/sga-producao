...
// >>> SUBSTITUA O BLOCO DE CONFIGURAÇÃO DO MULTER POR ESTE <<<
...
app.post('/api/asset-movements/substitute', authenticateToken, authorizePermission('DATA_MANAGE'), async (req, res) => {
  ...
  try {
    ...
    // --- 1. Validação dos Ativos e do Responsável ---
    ...
    // Encontra a última movimentação de saída/empréstimo do ativo antigo para saber quem é o responsável e qual o setor de destino
    const lastMovementRes = await client.query(
      `SELECT recipient_person_id, destination_sector_id FROM asset_movements 
    ...
    const { recipient_person_id, destination_sector_id } = lastMovementRes.rows[0];

    // --- 2. Processa a DEVOLUÇÃO do Ativo Antigo ---
    ...
    // Busca o setor padrão "Almoxarifado" para devolver o ativo
    ...
    // --- 3. Processa a SAÍDA do Ativo Novo ---
    ...
    const exitMovementRes = await client.query(
      `INSERT INTO asset_movements (movement_type, responsible_user_id, recipient_person_id, destination_sector_id, notes, delivery_status) 
       VALUES ('exit', $1, $2, $3, $4, 'pending_confirmation') RETURNING id`,
      [responsibleUserId, recipient_person_id, destination_sector_id, exitNote]
    );
    ...
    await client.query('UPDATE assets SET status = $1, current_sector_id = $2 WHERE id = $3', ['in_use', destination_sector_id, newAssetId]);

    // --- 4. Vincula as duas movimentações (opcional, mas bom para rastreabilidade) ---
    ...
    await client.query('ROLLBACK'); // Desfaz tudo se houver qualquer erro
...
app.get('/api/asset-movements/active-loans', authenticateToken, async (req, res) => {
  ...
  // Esta query busca movimentações que são 'loan', e cujo ativo correspondente ainda está com o status 'loaned'.
  // Isso garante que não listemos empréstimos que já foram devolvidos.
  let query = `
    SELECT DISTINCT
