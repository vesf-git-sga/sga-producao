...
app.post('/api/asset-movements', authenticateToken, authorizeRole(['admin', 'manager']), async (req, res) => {
  const {
    asset_id,
    movement_type,
    recipient_person_id, // NOVO: ID da pessoa cadastrada
    recipient_name, // Mantido para compatibilidade ou caso não seja uma pessoa cadastrada
    recipient_document, // Mantido para compatibilidade
    purpose,
  ...
  try {
    ...
    const newMovement = await pool.query(
      `INSERT INTO asset_movements (asset_id, movement_type, responsible_user_id, recipient_person_id, recipient_name, recipient_document, purpose, expected_return_date, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
      [asset_id, movement_type, responsible_user_id, recipient_person_id || null, recipient_name || null, recipient_document || null, purpose || null, expected_return_date || null, notes || null]
    );
  ...
  try {
    const result = await pool.query(
      `SELECT
         ...
         p.full_name AS recipient_person_full_name, -- Nome da pessoa cadastrada
       ...
       LEFT JOIN people p ON am.recipient_person_id = p.id -- JOIN com a tabela people
...
app.get('/api/dashboard/summary', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const recentMovements = recentMovementsResult.rows.map(row => ({
      ...
      user: row.responsible_user_name || row.recipient_display_name || 'N/A' // Ajustado para usar o nome da pessoa
    }));
  ...
  try {
    const movementResult = await pool.query(
      `SELECT
         ...
         s.secretariat AS current_sector_secretariat,
         s.sector_name AS current_sector_name,
         p.full_name AS recipient_person_full_name, -- Adicionado nome completo da pessoa recebedora
         p.cpf AS recipient_person_cpf,
         p.registration_number AS recipient_person_registration
       FROM asset_movements am
       JOIN assets a ON am.asset_id = a.id
       ...
       JOIN users u ON am.responsible_user_id = u.id
       LEFT JOIN sectors s ON a.current_sector_id = s.id
       LEFT JOIN people p ON am.recipient_person_id = p.id -- Adicionado JOIN com people
       WHERE am.id = $1`,
      [id]
    );
      ...
      content: [
        ...
        { text: 'Detalhes da Movimentação:', style: 'sectionHeader' },
        {
          ul: [
            ...
            // Prioriza recipient_person_full_name se existir, senão usa recipient_name
            movement.recipient_person_full_name ? `Recebedor: ${movement.recipient_person_full_name}` : (movement.recipient_name ? `Recebedor: ${movement.recipient_name}` : null),
            movement.recipient_person_cpf ? `CPF Recebedor: ${movement.recipient_person_cpf}` : (movement.recipient_document ? `Documento Recebedor: ${movement.recipient_document}` : null),
            movement.recipient_person_registration ? `Matrícula Recebedor: ${movement.recipient_person_registration}` : null,
        ...
        { text: 'Detalhes do Ativo:', style: 'sectionHeader' },
        {
          ul: [
            ...
            `Status Atual do Ativo: ${movement.status ? movement.status.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'N/A'}`,
            `Setor Atual: ${movement.current_sector_secretariat || 'N/A'} - ${movement.current_sector_name || 'N/A'}`,
          ].filter(Boolean), // Filtra itens nulos
          margin: [0, 5]
        },
        { text: '\n\n' },
        ...
        { text: `${movement.recipient_person_full_name || movement.recipient_name || movement.responsible_user_full_name}`, alignment: 'center' },
