...
import {
  ...
  CheckCircle, XCircle, Info, AlertTriangle as AlertTriangleIcon, Repeat, FileText // Adicionado FileText para PDF
} from 'lucide-react';
...
const DashboardPage = () => {
  ...
  }, [user, authLoading, fetchAssets, fetchItemTypes, fetchSectors, fetchDashboardData, fetchMovements]);
  ...
  const handleGenerateMovementReceipt = async (movementId: number) => {
    try {
      ...
      // CORREÇÃO AQUI: O segundo argumento do Blob constructor deve ser um objeto de opções
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className={`fixed top-0 left-0 h-full bg-blue-900 text-white w-64 p-5 z-50 transform lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
        <div className="flex items-center justify-between mb-8">
          ...
          />
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-white focus:outline-none">
            <X className="w-6 h-6" />
          </button>
        </div>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {movements.map((movement: Movement) => (
                          <tr key={movement.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{movement.recipient_name || 'N/A'}</td>
...
const MovementModal = ({ onClose, onSave, assets, users }: MovementModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (movementType === 'loan' && !expectedReturnDate) {
      alert('A data de devolução esperada é obrigatória para empréstimos.');
      setLoading(false);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            >
              ...
              {filteredAssets.map((asset) => ( // Usando filteredAssets
          ...
          {(movementType === 'loan' || movementType === 'exit') && (
            <>
              <div>
                <label htmlFor="recipientName" className="block text-sm font-medium text-gray-700 mb-1">Nome do Recebedor</label>
                <input
                  type="text"
                  id="recipientName"
                  value={recipientName}
                  ...
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                  required={movementType === 'loan' || movementType === 'exit'}
                />
              </div>
              <div>
                <label htmlFor="recipientDocument" className="block text-sm font-medium text-gray-700 mb-1">Documento do Recebedor</label>
                <input
                  type="text"
          ...
          {/* CORREÇÃO AQUI: Data de Devolução Esperada só para Empréstimo e Manutenção */}
          {(movementType === 'loan' || movementType === 'maintenance') && (
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                required={movementType === 'loan'} // Apenas obrigatório para empréstimo
              />
            </div>
          )}
