...
import {
  ...
  CheckCircle, XCircle, Info, AlertTriangle as AlertTriangleIcon, Repeat, FileText, Users, Search // Adicionado Users e Search para pessoas e busca
} from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-[1001] p-4">
      <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-4xl relative overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <fieldset disabled={isLockedAfterSave}>
            ...
            <div>
              ...
              <select id="movementType" value={movementType} onChange={(e) => setMovementType(e.target.value as Movement['movement_type'])} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm disabled:bg-gray-100" required>
                ...
                {canReturnAssets && <option value="return">Devolução</option>}
          ...
          <div className="flex justify-end space-x-3 pt-4">
            <button type="button" onClick={onClose} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-lg shadow-sm hover:bg-gray-300">{lastMovementId ? 'Fechar' : 'Cancelar'}</button>
            {!lastMovementId ? (
              <button type="submit" disabled={loading} className="px-6 py-2 bg-blue-700 text-white rounded-lg shadow-sm hover:bg-blue-800 disabled:opacity-50">{loading ? 'Salvando...' : 'Registrar Movimentação'}</button>
            ) : isLockedAfterSave ? (
              <><button type="button" onClick={() => handleGenerateMovementReceipt(lastMovementId)} className="px-6 py-2 bg-purple-600 text-white rounded-lg shadow-sm hover:bg-purple-700 flex items-center"><FileText className="w-5 h-5 mr-2" /> Gerar Recibo</button><button type="button" onClick={resetForm} className="px-6 py-2 bg-green-600 text-white rounded-lg shadow-sm hover:bg-green-700 flex items-center"><PlusCircle className="w-5 h-5 mr-2" /> Nova Movimentação</button></>
            ) : (
              <><button type="button" onClick={() => handleGenerateMovementReceipt(lastMovementId)} className="px-6 py-2 bg-purple-600 text-white rounded-lg shadow-sm hover:bg-purple-700 flex items-center"><FileText className="w-5 h-5 mr-2" /> Gerar Recibo</button><button type="button" onClick={handleLockForm} className="px-6 py-2 bg-yellow-500 text-white rounded-lg shadow-sm hover:bg-yellow-600 flex items-center"><Lock className="w-5 h-5 mr-2" /> Confirmar e Travar</button></>
            )}
          </div>
        </form>
      </div>
