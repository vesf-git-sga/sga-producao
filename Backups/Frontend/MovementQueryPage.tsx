// Arquivo: src/components/MovementQueryPage.tsx
...
interface MovementQueryPageProps {
  ...
  onRegisterMovementClick: () => void; // <<< A PROPRIEDADE QUE ESTAVA FALTANDO
                ...
                <tbody>
                  {movements.map((movement: Movement) => (
                    <tr key={movement.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4"><span className={`px-2 py-1 font-semibold leading-tight text-xs rounded-full ${movement.movement_type === 'entry' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{translateMovementType(movement.movement_type)}</span></td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap"><button onClick={() => handleGenerateMovementReceipt(movement.id)} className="font-medium text-purple-600 hover:underline" title="Gerar Recibo PDF"><FileText className="inline-block w-5 h-5" /></button></td>
                    </tr>
                  ))}
                </tbody>
            </table>
