...
app.post('/api/asset-movements', authenticateToken, authorizeRole(['admin', 'manager']), async (req, res) => {
  ...
  try {
    ...
    const newMovementResult = await client.query(
      ...
      [
        movement_type, responsible_user_id, recipient_person_id || null, recipient_name || null,
        recipient_document || null, purpose || null, expected_return_date || null, notes || null,
        destination_sector_id || null, request_channel_type || null, request_channel_details || null
      ]
    );
    ...
    for (const asset_id of asset_ids) {
      // Verificar se o ativo existe e obter seu status atual
      ...
      // Validar a movimentação com base no status atual do ativo
      switch (movement_type) {
        case 'entry':
          newAssetStatus = 'available';
          // Para entrada, o setor atual do ativo deve ser o setor de destino da movimentação
          await client.query(
            `UPDATE assets SET status = $1, current_sector_id = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`,
            [newAssetStatus, destination_sector_id || null, asset_id]
          );
          break;
        case 'exit':
          ...
          // Para saída, o setor atual do ativo deve ser o setor de destino da movimentação
          await client.query(
            `UPDATE assets SET status = $1, current_sector_id = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`,
            [newAssetStatus, destination_sector_id || null, asset_id]
          );
        ...
        case 'loan':
          ...
          // Para empréstimo, o setor atual do ativo deve ser o setor de destino da movimentação
          await client.query(
            `UPDATE assets SET status = $1, current_sector_id = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`,
            [newAssetStatus, destination_sector_id || null, asset_id]
          );
        ...
        case 'return':
          ...
          newAssetStatus = 'available';
          // Para devolução, o setor atual do ativo deve ser o setor de destino da movimentação
          await client.query(
            `UPDATE assets SET status = $1, current_sector_id = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`,
            [newAssetStatus, destination_sector_id || null, asset_id]
          );
        ...
        case 'maintenance':
          ...
          // Para manutenção, o setor atual do ativo deve ser o setor de destino da movimentação
          await client.query(
            `UPDATE assets SET status = $1, current_sector_id = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`,
            [newAssetStatus, destination_sector_id || null, asset_id]
          );
      ...
      // Inserir na tabela de junção movement_assets
      await client.query(
    ...
    await logAudit(responsible_user_id, `create_movement_${movement_type}`, 'asset_movement', newMovementId, { asset_ids, movement_type, new_status: movement_type === 'return' ? 'available' : newAssetStatus }, ipAddress);
    ...
    await logAudit(responsible_user_id, `create_movement_error_${movement_type}`, 'asset_movement', null, { error: error.message, asset_ids, movement_type }, ipAddress);
    res.status(500).json({ message: `Erro interno do servidor ao registrar movimentação: ${error.message}` });
  } finally {
    if (client) {
      client.release();
    }
