...
async function getFilteredMovements(filters) {
  ...
  let baseQuery = `
    SELECT
     am.id, am.movement_type, am.movement_date, am.purpose, am.expected_return_date,
     am.actual_return_date, am.notes, am.created_at, am.updated_at,
...
`;
  ...
  // --- LÓGICA DE CONSTRUÇÃO DINÂMICA DA CONSULTA ---
  const subQueryConditions = [];
  if (patrimonio) {
    queryParams.push(`%${patrimonio}%`);
    subQueryConditions.push(`a.patrimonio_number ILIKE $${queryParams.length}`);
  }
  if (solicitante) {
    queryParams.push(`%${solicitante}%`);
    subQueryConditions.push(`p_sub.full_name ILIKE $${queryParams.length}`);
  }
  if (cpf) {
    queryParams.push(cpf);
    subQueryConditions.push(`p_sub.cpf = $${queryParams.length}`);
  }
  if (matricula) {
    queryParams.push(matricula);
    subQueryConditions.push(`p_sub.registration_number = $${queryParams.length}`);
  }

  if (subQueryConditions.length > 0) {
    whereClauses.push(`
      am.id IN (
        SELECT DISTINCT m_sub.id 
        FROM asset_movements m_sub
        LEFT JOIN movement_assets ma_sub ON m_sub.id = ma_sub.movement_id
        LEFT JOIN assets a ON ma_sub.asset_id = a.id
        LEFT JOIN people p_sub ON m_sub.recipient_person_id = p_sub.id
  ...
  baseQuery += ` ORDER BY am.movement_date DESC`;

  const result = await pool.query(baseQuery, queryParams);

  const movementsWithAssets = await Promise.all(result.rows.map(async (movement) => {
    const assetsResult = await pool.query(
      `SELECT a.sku, a.patrimonio_number, a.brand, a.model FROM movement_assets ma JOIN assets a ON ma.asset_id = a.id WHERE ma.movement_id = $1`,
      [movement.id]
    );
...
app.get('/api/asset-movements/:id/receipt-pdf', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const equipmentTableBody = [
      ['QTD', 'EQUIPAMENTO', 'SÉRIE', 'TOMBO'].map(h => ({ text: h, style: 'tableHeader' }))
    ];
    assets.forEach(asset => {
      equipmentTableBody.push([
        '1',
        `${asset.item_type_name} ${asset.brand} ${asset.model}`,
    ...
    const docDefinition = {
      ...
      content: [
        ...
        { text: `Obs.: ${movement.notes || 'Em atendimento ao pedido com documentação anexa a este termo.'}`, style: 'body', margin: [0, 10, 0, 20] },
...
// >>> NOVA ROTA <<<
// Rota para listar movimentações com entrega pendente (com filtros)
app.get('/api/asset-movements/pending-delivery', authenticateToken, async (req, res) => {
  const { solicitante, patrimonio } = req.query;
  let queryParams = [];
  let whereClauses = ["am.delivery_status = 'pending_confirmation'", "am.movement_type IN ('loan', 'exit')"];

  let baseQuery = `
    SELECT
       am.id, am.movement_type, am.movement_date, am.purpose,
  ...
  // Lógica de filtro
  if (patrimonio) {
    // Encontra movimentações que contenham o ativo com o patrimônio informado
    queryParams.push(`%${patrimonio}%`);
    whereClauses.push(`am.id IN (
        SELECT ma.movement_id FROM movement_assets ma
        JOIN assets a ON ma.asset_id = a.id
        WHERE a.patrimonio_number ILIKE $${queryParams.length}
    )`);
  }
  
  if (solicitante) {
      // Encontra movimentações cujo solicitante (pessoa ou nome avulso) corresponda à busca
      queryParams.push(`%${solicitante}%`);
      const solicitanteParamIndex = `$${queryParams.length}`;
      whereClauses.push(`(p.full_name ILIKE ${solicitanteParamIndex} OR am.recipient_name ILIKE ${solicitanteParamIndex})`);
  }
  ...
  try {
    ...
    const movementsWithAssets = await Promise.all(result.rows.map(async (movement) => {
      const assetsResult = await pool.query(
        `SELECT a.sku, a.patrimonio_number, a.brand, a.model FROM movement_assets ma 
         JOIN assets a ON ma.asset_id = a.id WHERE ma.movement_id = $1`,
        [movement.id]
      );
      return { ...movement, assets: assetsResult.rows };
    }));
    ...
    console.error('Erro ao buscar movimentações com entrega pendente:', error);
    res.status(500).json({ message: 'Erro interno do servidor.' });
  }
});

// >>> NOVA ROTA <<<
// Rota para confirmar a entrega e fazer upload do recibo assinado
// Usa o middleware 'uploadReceipt' para processar um único arquivo do campo 'receiptFile'
app.post('/api/asset-movements/:id/confirm-delivery', authenticateToken, uploadReceipt.single('receiptFile'), async (req, res) => {
  const { id } = req.params; // ID da movimentação
  const { actual_delivery_date } = req.body; // Data da entrega vinda do formulário
  const ipAddress = req.ip;
  ...
  try {
    const receiptPath = req.file.path; // Caminho do arquivo salvo pelo multer
    ...
    const result = await pool.query(
      `UPDATE asset_movements
       SET 
         ...
         receipt_path = $2,
         updated_at = CURRENT_TIMESTAMP
       WHERE id = $3 AND delivery_status = 'pending_confirmation'
       RETURNING id, movement_type`,
      [actual_delivery_date, receiptPath, id]
    );

    if (result.rowCount === 0) {
      // Se nenhuma linha foi atualizada, pode ser que a entrega já foi confirmada ou o ID não existe
      return res.status(404).json({ message: 'Movimentação não encontrada ou entrega já confirmada.' });
    }

    const updatedMovement = result.rows[0];
    await logAudit(req.user.id, 'delivery_confirmed', 'asset_movement', updatedMovement.id, { receipt_path: receiptPath, delivery_date: actual_delivery_date }, ipAddress);
    
    res.status(200).json({ message: 'Entrega confirmada com sucesso!', movement: updatedMovement });

  } catch (error) {
    console.error('Erro ao confirmar entrega:', error);
    await logAudit(req.user.id, 'delivery_confirmation_error', 'asset_movement', id, { error: error.message }, ipAddress);
    res.status(500).json({ message: 'Erro interno do servidor ao confirmar a entrega.' });
  }
});

// Rota para obter estatísticas, como a contagem de entregas pendentes
app.get('/api/asset-movements/stats/pending-delivery-count', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT COUNT(*) AS pending_count 
       FROM asset_movements 
       WHERE delivery_status = 'pending_confirmation' AND movement_type IN ('loan', 'exit')`
    );
    const count = parseInt(result.rows[0].pending_count, 10);
    res.status(200).json({ count });
  } catch (error) {
    console.error('Erro ao contar entregas pendentes:', error);
    res.status(500).json({ message: 'Erro interno do servidor.' });
  }
});
